...
void ExternalBounce::setInterval(uint32_t interval_millis)
{
    this->interval_millis = interval_millis;
}

bool ExternalBounce::update(bool currentState, uint32_t currentMillis)
{
#ifdef BOUNCE_LOCK_OUT
    state &= ~_BV(STATE_CHANGED);
    // Ignore everything if we are locked out
    if (currentMillis - previous_millis >= interval_millis) {
